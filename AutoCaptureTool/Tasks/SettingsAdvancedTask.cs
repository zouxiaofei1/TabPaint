...
{
    public class SettingsAdvancedTask : ICaptureTask
    {
        public string Name => "设置-高级页";
        public CaptureMode TargetMode => CaptureMode.SettingsAdvanced;
        ...
        public void Execute(Window mainWindow, UIA3Automation automation, int processId, string themeName, string langName)
        {
            ...
            if (settingsWindow != null)
            {
                ...
                var navItem = settingsWindow.FindFirstDescendant(cf => cf.ByAutomationId("TxtAdvanced"));
                if (navItem != null)
                {
                    Console.WriteLine("找到导航项: 高级，正在切换页面...");
                ...
                {
                    string fileName = $"Settings_Advanced_{langName}_{themeName}.webp";
