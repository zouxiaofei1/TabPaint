...
{
    public class SettingsViewTask : ICaptureTask
    {
        public string Name => "设置-看图页";
        public CaptureMode TargetMode => CaptureMode.SettingsView;
        ...
        public void Execute(Window mainWindow, UIA3Automation automation, int processId, string themeName, string langName)
        {
            ...
            if (settingsWindow != null)
            {
                ...
                var navItem = settingsWindow.FindFirstDescendant(cf => cf.ByAutomationId("TxtView"));
                if (navItem != null)
                {
                    Console.WriteLine("找到导航项: 看图，正在切换页面...");
                ...
                {
                    string fileName = $"Settings_View_{langName}_{themeName}.webp";
